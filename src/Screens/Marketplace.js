...
export default function Marketplace() {
  ...
  const createFilters = (data) => {
    ...
    data.forEach((element) => {
      makerSet.add(element.MakerName);
      bodyTypeSet.add(element.body_type);
    });
    const filters = [
      {
        id: 'MakerName',
  ...
  return (
    <>
      {loading ? (
        <div className="loader w-16 h-16 border-4 border-blue-500 rounded-full border-t-transparent animate-spin-slow"></div>
      ) : (
